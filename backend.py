...
def get_job_work_details(job_id):
    ...
    try:
        ...
        if finished_job_data:
            # If job is finished, use stored values (remaining time could be negative)
            estimated_time = finished_job_data[0]
            remaining_time = finished_job_data[1]  # Keep stored value (could be negative)
            ...
            remaining_time = estimated_time - total_hours_worked  # Can be negative
        ...
        # For finished jobs, ensure remaining time matches stored value (even if negative)
        if finished_job_data:
            remaining_time = finished_job_data[1]
        else:
            remaining_time = estimated_time - total_worked  # Can be negative

        return {
            'jobId': job_id,
            'estimatedTime': estimated_time,
            'users': users,
            'remainingTime': round(remaining_time,2),  # Can be negative
    ...
    def do_GET(self):
            ...
            try:
                ...
                for row in rows:
                    record = {
                        ...
                        'remainingTime': round(max(float(row[11]) - float(row[12]), 0.0),2),
                        'status' : row[13]
                    }
        ...
        # In do_GET method
            ...
            try:
                ...
                # --- Fetch Production Ready Date ---
                production_ready_date = None
                if status == "Completed":
                    cursor.execute("""
                        SELECT [Production Ready Date]
                        FROM Jobsfinished
                        WHERE PN = ?
                        ORDER BY ROWID DESC
                        LIMIT 1
                    ...
                    if row_prd and row_prd[0]:
                        production_ready_date = row_prd[0]
                else:
                    cursor.execute("""
                        SELECT [Production Ready Date]
                        FROM PN_DATA
                ...
                # --- Fetch Backorder flag from PN_DATA ---
                backorder = False
                try:
                    cursor.execute('SELECT COALESCE(backorder, 0) FROM PN_DATA WHERE PN = ? LIMIT 1', (job_id,))
                    row_bo = cursor.fetchone()
                    if row_bo:
                        s = str(row_bo[0]).strip().lower()
                        backorder = s in ("1", "y", "yes", "true", "t")
                except Exception as e:
                    print("WARN: Failed to read Backorder flag:", e)
                ...
                # Merge common fields into pn_data
                pn_data["backorder"] = backorder
                pn_data["productionReadyDate"] = production_ready_date
                ...
                if row:
                    ...
                    total_labor = float(row[3] or 0) if status == "Completed" else None
                    profit = float(row[4] or 0) if status == "Completed" else None
                    staff_json = row[5] or "{}"
                else:
                    try:
                        av = float(pn_data.get("av") or pn_data.get("AV") or 0)
                        qty = float(pn_data.get("qty") or pn_data.get("QTY") or 0)
                        estimated_time = round(av * qty, 2)
                    except:
                        estimated_time = 0
                ...
                try:
                    ...
                    for k, v in staff_dict.items():
                        if isinstance(v, str):
                            ...
                            v = {"worked_hours": v}
                        hours = float(v.get("worked_hours", 0))
                        if hours > 0:
                            users.append({"name": k, "hours": hours})
                except:
                    pass
                ...
                logs = [{"staffName": r[0], "startTime": r[1], "stopTime": r[2] or "In Progress", "hours": r[3]} for r in log_rows]
                ...
                response = {
                    "pnData": {
                        ...
                        "productionReadyDate": pn_data.get("productionReadyDate"),
                        "backorder": pn_data.get("backorder"),
                        "s$": s_price,
                        "b$": b_price
                    },
                    "workData": {
    ...
    def do_POST(self):
        ...
        if self.path == '/clock-in':
            ...
            try:
                ...
                # Automatically stop any active jobs for this staff
                stop_time = get_current_timestamp()
                cursor.execute('''
                    UPDATE ClockInOut
                    SET StopTime = ?
                    WHERE StaffName = ? AND StopTime IS NULL
                ''', (stop_time, staff_name))
            ...
            try:
                ...
                with sqlite3.connect('prod_management.db') as conn:
                    cursor = conn.cursor()
                    cursor.execute('''
                        UPDATE ClockInOut
                        SET StopTime = ?
            ...
            try:
                ...
                # Load current staff list from the CSV
                staff_file = 'staff.csv'
                with open(staff_file, mode='r') as file:
                    lines = file.readlines()

                # Filter out the staff name to delete
                with open(staff_file, mode='w', newline='') as file:
                    writer = csv.writer(file)
                    for line in lines:
                        if line.strip().lower() != staff_name_to_delete.lower():
                ...
                self.send_header('Content-Type', 'application/json')
                self.end_headers()
                self.wfile.write(json.dumps({'message': 'Staff deleted successfully'}).encode('utf-8'))
            ...
            try:
                ...
                plan_ready_date = data.get('planReadyDate', '')  # new field
                ...
                try:
                    cursor.execute('''
                        INSERT INTO PN_DATA (PN, "INPUT DATE", "NO/CELL", "DRAW NO", "REQU-DATE", "CUST", 
                                            ...
                                            "TEST TIME", "AV", "S$", "DISCOUNT", "SALESMAN", "Customer Code", "Order Date","EXCLUDE_SAVE_TIME","PLAN READY DATE") 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?)
                    ''', (job_id, input_date, no_cell, draw_no, req_date, cust, stock_code, qty, cell_code, b_price, 
                        ...
                        av, s_price, discount, salesman, customer_code, order_date,exclude_save_time, plan_ready_date))
                    conn.commit()
            ...
            try:
                ...
                conn = sqlite3.connect('prod_management.db')  # Replace with your actual DB connection
                cursor = conn.cursor()
        ...
        #EDIT clock in/out
            ...
            try:
                ...
                # Fetch the JobID before deleting
                cursor.execute('SELECT JobID FROM ClockInOut WHERE RecordID = ?', (record_id,))
                row = cursor.fetchone()
            ...
            try:
                ...
                for item in records:
                    ...
                    values.append(pn)
                    query = f'UPDATE TestRecords SET {set_clause} WHERE PN = ?'
                ...
                print(f"Error in /update-multiple-test-records: {e}")
                self.send_response(500)
                self.set_cors_headers()
                self.end_headers()
                self.wfile.write(json.dumps({"error": str(e)}).encode())
                ...
                try:
                    ...
                    cursor.execute('''
                        INSERT INTO JOBSFINISHED (
                            ...
                            TotalHoursWorked, RemainingTime, Status, EXCLUDE_SAVE_TIME, "PRODUCTION READY DATE","PLAN READY DATE"
                        )
                        SELECT 
                            "INPUT DATE", PN, "NO/CELL", "DRAW NO", "REQU-DATE", CUST, "STOCK CODE", 
                            QTY, "CELL CODE", "B$", "ORDER NO", MODEL, VOL, AH, WH, CHEM, STRUCTURE, 
                            ...
                            "C-CELLS", "C-AV", "C-B$", "C-S$", "C-STCODE", "ORIGINAL S$", DISCOUNT, 
                            SALESMAN, "Customer Code", "Order Date", ?, ?, ?, ?, 'Completed', EXCLUDE_SAVE_TIME, "PRODUCTION READY DATE","PLAN READY DATE"
                    ...
                    cursor.execute('''
                        INSERT OR IGNORE INTO TestRecords (
                            ...
                            total_labor_cost, salesman, customer_code, order_no, profit,"PLAN READY DATE", staff_details
                        )
                        SELECT 
                            j."END DATE" as date,
                            j.PN,
    ...
    def handle_get_test_records(self):
        try:
            ...
            for row in cursor.fetchall():
                ...
                print(f"📦 Fetched Record {record['PN']} -> has_pdf: {record.get('has_pdf')}, picture: {record.get('picture')}")
                ...
                record['profit'] = round(
                    ...
                    (record['qty'] * record['bill_price']), 2
                )
               ...
               # 🔍 Check if the job is marked as special
                # 🔍 Check if this job is a special one
                cursor.execute("SELECT EXCLUDE_SAVE_TIME FROM JOBSFINISHED WHERE PN = ?", (record['PN'],))
                row = cursor.fetchone()
                is_special = row and row[0] == 1
                print(f"🧪 Job {record['PN']} is special? {is_special}")
        ...
        # 🔍 Fetch staff work details
                ...
                # ✅ Build staff_details with or without saveTime
                staff_details = {}
                for staff_name, worked_hours in staff_work_details:
                    if is_special:
                        save_time = ""
                    else:
                        save_time = round((worked_hours / record['total_time']) * record['remaining_time'], 2) if record['total_time'] != 0 else 0
                ...
                print(f"✅ Updating DB with staff_details for {record['PN']}")
                cursor.execute("""
                    UPDATE TestRecords SET staff_details = ? WHERE PN = ?
                """, (record['staff_details'], record['PN']))
                conn.commit()

                print(f"✅ Staff for {record['PN']}: {staff_details}")
                    

                    #if record['total_time']!= 0 :
                    #    save_time = round((worked_hours / record['total_time']) * record['remaining_time'], 2)
                    #staff_details[staff_name] = {'worked_hours': worked_hours, 'save_time': save_time}

                # Convert staff details to JSON and add to the record
                #record['staff_details'] = json.dumps(staff_details)
