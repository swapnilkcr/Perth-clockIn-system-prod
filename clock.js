...
async function handleJobStart(staffName, jobId) {
  try {
    const response = await fetch(`${BASE_URL}/start-job`, {
      ...
      body: JSON.stringify({ 
        staffName,
        jobId,
        requestTime: new Date().toISOString() // For debugging
      })
    });

      const data = await response.json();
      console.log('Staff name:', JSON.stringify(staffSelect.value));
      console.log('Job ID:', JSON.stringify(jobSelect.value));
    if (!response.ok) {
      // Special handling for active job conflict
      if (data.code === 'ACTIVE_JOB_EXISTS') {
        const activeJobId = data.error.match(/job (\d+)/)[1];
        ...
        if (userConfirmed) {
          // First stop the active job
          const stopResponse = await fetch(`${BASE_URL}/stop-job`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
          ...
          if (!stopResponse.ok) {
            throw new Error('Failed to stop current job');
          }
          
          // Then retry starting the new job
          return handleJobStart(staffName, jobId);
        }
        return { cancelled: true }; // User chose not to proceed
      }
...
// Update your start button event listener
startBtn.addEventListener('click', async (event) => {
  ...
  if (!staffName || !jobId) {
    showMessage('Please select both staff and job', 'error');
    return;
  }

  const result = await handleJobStart(staffName, jobId);
  ...
  fetch(`${BASE_URL}/view-running-jobs`, {
    ...
    .then(data => {
      ...
      if (data.runningJobs && data.runningJobs.length > 0) {
        ...
        resetTableHeaders(['Staff Name', 'Job ID', 'Customer', 'Drawing Number', 'Start Time', 'Stop Button']);
    ...
    fetch(`${BASE_URL}/add-staff`, {
      ...
      .then(data => {
        if (data.message) {
          ...
          //document.getElementById('staffModal').style.display = 'none'; //closes the modal
